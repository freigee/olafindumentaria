  ...
  ready(function () {
    ...
    safe(function initProductos() {
      ...
      function parseCSV(text) {
        ...
        var headers = lines[0].split(',').map(function(h){ return h.trim().replace(/^"|"$/g,'').toLowerCase().replace(/\s+/g,''); });
      ...
      /* ── Lightbox: foto completa + scroll para ver el resto ── */
      var lightbox = document.createElement('div');
      lightbox.className = 'prod-lightbox';
      lightbox.innerHTML =
        '<button type="button" class="prod-lightbox__close" aria-label="Cerrar">✕</button>' +
        '<div class="prod-lightbox__scroll"></div>';
      document.body.appendChild(lightbox);
      var lightboxScroll = lightbox.querySelector('.prod-lightbox__scroll');

      function abrirLightbox(imgs, nombre, startIdx) {
        lightboxScroll.innerHTML = imgs.map(function(src){
          return '<img src="'+src+'" alt="'+nombre+'">';
        }).join('');
        lightbox.classList.add('is-open');
        document.body.style.overflow = 'hidden';
        var first = lightboxScroll.children[startIdx || 0];
      ...
      lightbox.querySelector('.prod-lightbox__close').addEventListener('click', cerrarLightbox);
      ...
      function crearCard(p) {
        ...
        var imgs = [p.imagen, p.imagen2, p.imagen3, p.imagen4].filter(Boolean).map(fixImg);
        var article = document.createElement('article');
        article.className = 'prod-card is-visible';
        article.dataset.categoria = p.categoria || '';

        var imgsHtml = imgs.map(function(src, i) {
          return '<img src="'+src+'" alt="'+p.nombre+'" loading="lazy" class="prod-card__img'+(i===0?' is-active':'')+'" onerror="this.remove()">';
        }).join('');

        var dotsHtml = imgs.length > 1
          ? '<div class="prod-card__dots">' + imgs.map(function(_, i) {
              return '<button type="button" class="prod-card__dot'+(i===0?' is-active':'')+'" data-i="'+i+'" aria-label="Ver foto '+(i+1)+'"></button>';
            }).join('') + '</div>'
          : '';

        article.innerHTML =
          '<div class="prod-card__img-wrap'+(imgs.length?'':' no-img')+(imgs.length>1?' has-multi':'')+'">' +
            imgsHtml +
        ...
        var imgWrap = article.querySelector('.prod-card__img-wrap');
        var current = 0;

        if (imgs.length > 1) {
          var imgEls = article.querySelectorAll('.prod-card__img');
          var dotEls = article.querySelectorAll('.prod-card__dot');
          var irA = function(i) {
            current = (i + imgEls.length) % imgEls.length;
            imgEls.forEach(function(im, idx){ im.classList.toggle('is-active', idx===current); });
            dotEls.forEach(function(d, idx){ d.classList.toggle('is-active', idx===current); });
          };
          dotEls.forEach(function(dot) {
            dot.addEventListener('click', function(e) {
              e.stopPropagation();
              irA(parseInt(dot.dataset.i, 10));
            });
          });
        }
        ...
        if (imgs.length) {
          imgWrap.addEventListener('click', function(e) {
            if (e.target.closest('.prod-card__dot')) return;
            abrirLightbox(imgs, p.nombre, current);
          });
        }
