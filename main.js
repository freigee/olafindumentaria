  ...
  ready(function () {
    ...
    safe(function initProductos() {
      ...
      var COLOR_MAP = {
        ...
        /* variantes en femenino */
        'negra':'#1A1A1A','blanca':'#F5F5F5','roja':'#C0272D','amarilla':'#E8C547','marrona':'#6B4A2F','marrona ':'#6B4A2F',
        /* tonos de azul */
        'azul claro':'#6FA8DC','azul oscuro':'#17325C','azul acido':'#3A6FD8','azul ácido':'#3A6FD8',
        'azulino':'#3F5FA8','francia':'#2A5CAA','azul francia':'#2A5CAA','cielo':'#8CC5E8','aero':'#7FB3D5',
        /* rojos y rosados */
        'borravino':'#5C1F2E','cereza':'#B3242B','ladrillo':'#A64B36','salmon':'#F09A84','salmón':'#F09A84',
        /* tierras y neutros */
        'arena':'#D6C3A1','topo':'#8A7A6D','terra':'#9C4A2F','terracota':'#9C4A2F',
        'gris melange':'#9E9E99','gris melangé':'#9E9E99','gris plomo':'#5A5F63',
        /* otros */
        'lavanda':'#B9A7D6','pastel':'#E0D3E4','verde oxido':'#5A6B4A','verde óxido':'#5A6B4A',
        /* typos frecuentes del catálogo */
        'verdse':'#3E6B35','nero':'#1A1A1A'
      };
      ...
      var pm = (function() {
        ...
        el.innerHTML =
          ...
          '<div class="pm__box">' +
            '<button class="pm__close" data-pm-close aria-label="Cerrar">✕</button>' +
            '<div class="pm__gallery"></div>' +
            '<div class="pm__info">' +
              '<span class="pm__cat mono"></span>' +
              '<h2 class="pm__name"></h2>' +
              '<p class="pm__price"></p>' +
              '<div class="pm__block pm__block--colores" hidden>' +
                '<span class="pm__label mono">Color</span>' +
                '<div class="pm__opts pm__colores"></div>' +
              '</div>' +
              '<div class="pm__block pm__block--talles" hidden>' +
                '<span class="pm__label mono">Talle</span>' +
                '<div class="pm__opts pm__talles"></div>' +
              '</div>' +
              '<button class="btn btn--primary pm__add"></button>' +
              '<p class="pm__hint mono">Coordinamos disponibilidad y entrega por WhatsApp.</p>' +
            '</div>' +
          '</div>';
        document.body.appendChild(el);

        var gal     = el.querySelector('.pm__gallery');
        var infoEl  = el.querySelector('.pm__info');
        var catEl   = el.querySelector('.pm__cat');
        var nameEl  = el.querySelector('.pm__name');
        var priceEl = el.querySelector('.pm__price');
        var colWrap = el.querySelector('.pm__block--colores');
        var colEl   = el.querySelector('.pm__colores');
        var talWrap = el.querySelector('.pm__block--talles');
        var talEl   = el.querySelector('.pm__talles');
        var addBtn  = el.querySelector('.pm__add');

        var imgs = [], nombre = '', precio = 0, talleSel = '', colorSel = '', imgSel = '', colorIdx = -1;

        // lleva la galería a la foto N. Las fotos de Drive cargan de a poco, así que
        // esto se vuelve a llamar en cada load hasta que la posición sea la correcta.
        function irAFoto(i) {
          var target = gal.children[i];
          if (target && target.tagName === 'IMG') gal.scrollTop = target.offsetTop;
        }

        function refrescarBoton() {
          var falta = !talWrap.hidden && !talleSel;
        ...
        function open(p) {
          ...
          imgs     = getImgs(p);
          nombre   = p.nombre || '';
          precio   = parseInt(String(p.precio || '0').replace(/\D/g,''), 10) || 0;
          talleSel = ''; colorSel = ''; imgSel = imgs[0] || ''; colorIdx = -1;

          gal.innerHTML = imgs.length
            ? imgs.map(function(src){ return '<img src="'+esc(src)+'" alt="'+esc(nombre)+'">'; }).join('')
            : '<div class="pm__noimg mono">Sin foto</div>';

          // al cargar cada foto el alto cambia: reubicarse en la foto del color elegido
          gal.querySelectorAll('img').forEach(function(im) {
            im.addEventListener('load', function(){ if (colorIdx > 0) irAFoto(colorIdx); });
          });

          catEl.textContent = p.categoria || '';
          catEl.hidden = !p.categoria;
          nameEl.textContent = nombre;
          var precioTxt = fmtPrecio(p.precio);
          priceEl.textContent = precioTxt || '';
          priceEl.hidden = !precioTxt;

          colWrap.hidden = !colores.length;
          colEl.innerHTML = colores.map(function(c, i) {
            var hex = colorCSS(c);
            var st  = hex ? ' style="background:'+hex+'; color:'+textoSobre(hex)+'; border-color:'+hex+';"' : '';
            return '<button class="color-btn" data-color="'+esc(c)+'" data-idx="'+i+'"'+st+'>'+esc(c)+'</button>';
          }).join('');

          talWrap.hidden = !talles.length;
          talEl.innerHTML = talles.map(function(t) {
            return '<button class="talle-btn" data-talle="'+esc(t)+'">'+esc(t)+'</button>';
          }).join('');

          refrescarBoton();
          el.hidden = false;
          gal.scrollTop = 0; infoEl.scrollTop = 0;
        ...
        el.addEventListener('click', function(e) {
          ...
          var col = e.target.closest('.color-btn');
          if (col) {
            colEl.querySelectorAll('.color-btn').forEach(function(b){ b.classList.remove('active'); });
            col.classList.add('active');
            colorSel = col.dataset.color;
            var i = parseInt(col.dataset.idx, 10);
            if (imgs[i]) imgSel = imgs[i];
            colorIdx = i;
            irAFoto(i);
            return;
          }

          var tal = e.target.closest('.talle-btn');
          if (tal) {
            talEl.querySelectorAll('.talle-btn').forEach(function(b){ b.classList.remove('active'); });
            tal.classList.add('active');
            talleSel = tal.dataset.talle;
            refrescarBoton();
            return;
          }

          if (e.target.closest('.pm__gallery img')) { lb.open(imgs, nombre); return; }

          if (e.target.closest('.pm__add') && !addBtn.disabled) {
            window.olafAgregarCarrito({
              nombre: nombre, precio: precio,
              talle: talleSel, color: colorSel, imagen: imgSel
            });
            close();
          }
        });
      ...
      function crearCard(p) {
        ...
        // click en la tarjeta → abre la vista rápida (salvo si tocó un control)
        article.addEventListener('click', function(e) {
          if (e.target.closest('.color-btn, .talle-btn, .prod-card__add')) return;
          pm.open(p);
        });
        ...
        article.querySelector('.prod-card__add').addEventListener('click', function(e) {
          ...
          window.olafAgregarCarrito({
            ...
            precio: parseInt(btn.dataset.precio,10)||0,
            talle: btn.dataset.talle||'',
            color: btn.dataset.color||'',
            imagen: (imgEl && imgEl.src) || imgs[0] || ''
          });
      ...
      function aplicarFiltros() {
        var lista = todos.filter(function(p) {
          var gen = normalizarGenero(p.genero);
          var okGen;
          if (generoActivo==='todos') okGen = true;
          else if (generoActivo==='accesorios') okGen = (gen==='accesorios');
          else okGen = (gen===generoActivo || gen==='unisex');
      ...
      function buildFiltros(lista) {
        ...
        if (hayGenero && filtrosEl) {
          ...
          genWrap.innerHTML =
            ...
            '<button class="genero-btn" data-gen="accesorios">Accesorios</button>';
    ...
    safe(function initCarrito() {
      ...
      function render() {
        ...
        carrito.forEach(function(item,idx) {
          ...
          div.innerHTML =
            (item.imagen ? '<img class="cart-item__img" src="'+item.imagen+'" alt="" loading="lazy">' : '') +
      ...
      window.olafAgregarCarrito = function(item) {
        ...
        if (ex) { ex.cantidad++; if (!ex.imagen && item.imagen) ex.imagen = item.imagen; }
        else { carrito.push({nombre:item.nombre,precio:item.precio,talle:item.talle||'',color:item.color||'',imagen:item.imagen||'',cantidad:1}); }
      ...
      checkoutBtn && checkoutBtn.addEventListener('click', function() {
        ...
        var wa = (window.__OLAF__&&window.__OLAF__.brand&&window.__OLAF__.brand.whatsapp)
          ? window.__OLAF__.brand.whatsapp.replace(/\D/g,'') : '5491139309729';
        var total = carrito.reduce(function(a,i){ return a+i.precio*i.cantidad; },0);
        var lineas = carrito.map(function(i) {
          var l = '• '+i.nombre+(i.talle?' — Talle '+i.talle:'')+(i.color?' — Color '+i.color:'')+' x'+i.cantidad+' → '+fmt(i.precio*i.cantidad);
          if (i.imagen) l += '\n  Foto: '+i.imagen;
          return l;
        });
